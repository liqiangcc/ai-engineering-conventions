...
目标不是增加架构仪式，而是降低代码库的结构熵，让开发者、Reviewer 和 AI 对同一类问题具有相同预期，并能从外部入口稳定推导到业务实现、验证证据、部署结果和变更历史。

## 核心目标

- **可推导**：从 URL / operationId 可以大致推导业务模块、Use Case 和 HTTP Test。
- **可辨识**：从类名可以判断业务语义与架构职责。
- **可分离**：业务规则、流程控制、外部技术、测试和部署验证具有明确边界。
- **可验证**：从 Acceptance Criteria / BR / Contract 可以推导测试，并明确已验证与未验证范围。
- **可复现**：Bug 优先留下 `Before Fix: FAIL → After Fix: PASS` 的可执行证据，并永久保留为回归资产。
- **可部署验证**：区分代码已验证与真实运行实例已验证，Post-Deployment 必须有环境和运行版本证据。
- **可追踪**：Use Case、Business Rule、Business Change、Incident 可以通过稳定标识关联。
- **可 Review**：代码位置、命名、HTTP 场景、测试证据和变更范围具有一致规律。
- **可复盘**：Bug 可以沿入口 → 用例 → 规则 → 复现测试 → 修复 → 部署验证 → 历史追溯。
...
- [Bug Reproduction 与永久回归](docs/testing/bug-reproduction-testing.md)
...
2. 一份文档只解决一个主要问题。
3. 一个提交只承载一种主要变化原因。
4. 当前规则、验证证据、部署证据、历史变更和事故记录分开维护。
5. 可以自动推导的信息不重复手工维护。
6. 测试预期来自 AC / BR / Contract，不从当前实现自证正确。
7. HTTP Case 应成为可读、可 Git 管理、可人工/AI/CI 重放的工程资产。
8. Bug 修复后复现 Case 不删除，转化为永久 Regression Asset。
9. Pre-Deployment PASS 不等于 Post-Deployment VERIFIED。
10. 生产环境只执行 production-safe 验证，不能为了“全绿”破坏真实业务数据。
11. 先建立稳定边界，再增加工具和自动化。
12. AI 生成成本低不代表认知复杂度免费；优先选择重复但可预测的结构。

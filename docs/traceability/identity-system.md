...
唯一性针对逻辑身份定义，不针对 ID 字符串的出现次数。这样可隔离身份冲突与正常导航引用，避免扫描器把测试、注解和文档的重复引用误判为重复业务概念。

- UC 的主要身份定义是主要 Use Case 的声明；多个入口引用同一个 UC 合法，两个独立主要 Use Case 声明同一个 UC ID 则失败。
- BR 身份定义是当前逻辑规则的声明。已有 BR 文档时，该文档是规则语义的权威定义，代码注解和测试元数据是实现/验证关联；无 BR 文档时，项目必须说明如何从现有规则元数据识别唯一逻辑定义，不为扫描器额外维护重复索引。
- 同一 BR 可以对应多个实现位置；重复注解、文档引用、HTTP 引用本身不构成身份冲突。
- 两个独立规则定义复用同一 ID，或声明元数据相互冲突，MUST 失败；不比较源文本出现次数，也不能任意选择一个定义。
- 工具无法根据项目已有权威源区分定义与引用时，应报告 NOT_VERIFIED 并说明缺少什么依据，不能宣称唯一性检查通过。业务含义是否相同不能仅由正则判断。

| 场景 | 唯一性判断 |
|---|---|
| 一条 BR 文档被实现、测试、HTTP 引用 | 合法 |
| 同一 BR 明确关联两个实现位置 | 合法 |
| 两个主要 Use Case 声明同一 UC ID | 失败 |
| 两份独立 BR 定义复用同一 ID | 失败 |
| 同一身份声明存在冲突元数据 | 失败，定位冲突来源 |
| 只有多次 ID 出现，无法识别定义或引用 | NOT_VERIFIED，不能按次数判冲突 |
